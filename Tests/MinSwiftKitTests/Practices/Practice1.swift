...
class Practice1: ParserTestCase {
    ...
    func testPeek() {
        ...
        XCTAssertEqual(parser.read().tokenKind, .funcKeyword)
        XCTAssertEqual(parser.peek(1).tokenKind, .identifier("sayHello"))
        XCTAssertEqual(parser.peek(2).tokenKind, .leftParen)
        XCTAssertEqual(parser.peek(3).tokenKind, .rightParen)
        XCTAssertEqual(parser.peek(9).tokenKind, .rightBrace)
